 ...
 */
export function findNearest(lon, lat, lon2, lat2, limDist) {
 ...
 */
export function findNearestWay(lon, lat, lon2, lat2, limDist) {
 ...
 */
export function addNewCoordinate(lon, lat, lon2, lat2, index) {
...
export function getPerp(X1, Y1, X2, Y2, X3, Y3) {
  const XX = X2 - X1;
  const YY = Y2 - Y1;
  if (XX * XX + YY * YY === 0) {
    return [X2, Y2, 1];
