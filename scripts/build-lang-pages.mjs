/*
 * build-lang-pages.mjs — post-build generation of the per-language entry
 * pages for SEO. The root index.html is the French page; en.html, de.html,
 * es.html and it.html are copies with the <html lang>, title, description,
 * Open Graph tags and canonical swapped to the target language (the values
 * come from js/locales/<lang>.json). They live next to index.html so the
 * relative asset paths and the service worker keep working, and the
 * hreflang alternates (static in index.html, absolute URLs) are identical
 * on every page, as the spec requires. At runtime the app detects the
 * page language from the URL (see urlLang() in js/i18n.js).
 */
