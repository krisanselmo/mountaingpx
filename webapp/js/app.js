...
(function () {
  ...
  const { POI, GROUPS, DEFAULT_WITH_NAME } = window.POIData;
  ...
  function buildPoiPanel() {
    ...
    for (const [gkey, gname] of Object.entries(GROUPS)) {
      ...
      for (const [type, cfg] of (groups[gkey] || [])) {
        ...
        tdName.innerHTML = `${window.Icons.svgFor(type, 18)} ${cfg.label}`;
  ...
  function initWaterOverlay(map) {
    ...
    async function refresh() {
      ...
      loading = true;
      try {
        const json = await window.Overpass.run(window.Overpass.buildQuery(box, WATER_FILTERS));
        ...
        for (const el of json.elements) {
          ...
          const marker = L.marker([el.lat, el.lon], {
            ...
            icon: window.Icons.waterDotIcon(),
          });
          marker.bindPopup(
            `<div class="wpt-popup"><h3>${escapeHtml(tags.name || "Point d'eau")}</h3>` +
            window.Overpass.describeOsm(el.type, el.id, tags) + '</div>'
          );
          markers.push(marker);
  ...
  function drawRoute() {
    ...
    const mk = (kind, ll, title) =>
      L.marker(ll, { title, icon: window.Icons.flagIcon(kind) });
  ...
  function drawWaypoints() {
    ...
    for (const p of state.pts) {
      const marker = L.marker([p.lat, p.lon], {
        title: p.name,
        icon: window.Icons.markerIcon(p.queryName),
      });
  ...
  function renderProfileWaypoints() {
    ...
    for (const w of state.pts) {
      ...
      const color = window.Icons.GROUP_COLORS[group] || '#64748b';
  ...
  async function generate() {
    ...
    setBusy(true);
    try {
      const res = await window.Overpass.findWaypoints(state.route, sel, limDist, (done, total) => {
  ...
  function refreshFromMemory() {
    ...
    state.pts = applyOverrides(window.Overpass.snapElements(state.genElements, state.route, sel, limDist));
    syncWaypointUI();
    return true;
  }
  ...
  function updatePoiCounts() {
    ...
    const limDist = parseInt($('#snap-dist').value, 10) / 1000;
    const pts = window.Overpass.snapElements(
