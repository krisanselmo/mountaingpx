...
- Les marqueurs sont des `L.divIcon` SVG générés par `js/icons.js` — aucun
  asset image.
- Référencement : `index.html` porte les balises SEO statiques (description,
  canonical, Open Graph, JSON-LD, `hreflang`) ; `scripts/build-lang-pages.mjs`
  génère au build une page d'entrée par langue (`en.html`, `de.html`…) avec
  les meta traduites, `public/sitemap.xml` liste ces URL, et les préversions
  de PR sont marquées `noindex` (`NOINDEX_BUILD=1` dans `pr-preview.yml`).
